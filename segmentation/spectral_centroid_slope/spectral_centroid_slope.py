...
OUT_AUDIO_DIR = PROJECT_ROOT / "segmented_cycles_slope"

OUT_CSV = SCRIPT_DIR / "spectral_centroid_slope_segmentation.csv"
OUT_XLSX = SCRIPT_DIR / "spectral_centroid_slope_segmentation.xlsx"
...
def spectral_centroid_slope_segmentation(
    ...
    min_slope_percentile: float = 75.0
) -> None:
    ...
    for wav_path in wav_files:
        ...
        # slope = difference between consecutive centroid values
        slopes = np.diff(centroids)

        # keep only positive rises
        positive_slopes = np.maximum(slopes, 0)

        threshold = np.percentile(positive_slopes, min_slope_percentile)
        ...
        print(f"  Slope values: {len(slopes)}")
        print(f"  Slope threshold: {round(float(threshold), 2)}")
        ...
        for peak_idx, peak_frame in enumerate(peaks, start=1):
            ...
            cycle_filename = f"{wav_path.stem}_slope_cycle_{valid_cycles + 1}.wav"
            ...
            cycle_rows.append({
                ...
                "frame_ms": frame_ms,
                "hop_ms": hop_ms,
                "slope_threshold": round(float(threshold), 2),
                "peak_slope_value": round(float(positive_slopes[peak_frame]), 2)
            })
    ...
    print("\n[OK] Spectral Centroid Slope Segmentation finished.")
...
if __name__ == "__main__":
    spectral_centroid_slope_segmentation(
        cycle_length_s=2.5,
